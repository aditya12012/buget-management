...
function BreadcrumbAndProfile({ username, role, breadcrumbItems, pageTitle }) {
  ...
  if (pageTitle === 'Incomes') {
    welcomeMessage = `RS{username}, here are your incomes...`;
  } else if (pageTitle === 'Expenses') {
    welcomeMessage = `RS{username}, here are your expenses...`;
  } else if (pageTitle === 'Dashboard') {
    welcomeMessage = `Welcome back, RS{username}`;
  ...
  return (
    <>
      ...
      <div className="user-info d-flex justify-content-between align-items-center">
        ...
        <img src={`RS {process.env.PUBLIC_URL}/images/User/User.png`} alt={username} className="user-image" />
          <div>
